...
int main(){
    ...
    while (flag){
        flag = 0;
        int min_burst_time_indx = -1;

        for (int i = 0; i < n; i++){
            if (!processes[i].finished){
                flag = 1;
                if (processes[i].arrival_time <= current_time){
                    if (min_burst_time_indx == -1)
                        min_burst_time_indx = i;
                    else if (processes[min_burst_time_indx].burst_time > processes[i].burst_time)
                        min_burst_time_indx = i;
                }
            }
        }
        ...
        if (min_burst_time_indx == -1){
            ...
            int AT = processes[min_burst_time_indx].arrival_time;
            int BT = processes[min_burst_time_indx].burst_time;
            ...
            printf("P%d\t%d\t%d\t%d\t%d\t%d\n", processes[min_burst_time_indx].process_num, AT, BT, CT, TAT, WT);

            processes[min_burst_time_indx].finished = 1;
        }
    }
